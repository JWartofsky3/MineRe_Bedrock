...
export const useEmeraldStaff = (data) => {
    if (!data.source) {
        return;
    }
    ...
    if (itemStack.typeId == "minere:emerald_staff") {
        system.run(() => {
            ...
            const targetLocation = source.getBlockFromViewDirection({
                maxDistance: MAX_RANGE,
                includeLiquidBlocks: false,
                includePassableBlocks: false,
            })?.block?.location ??
                addVector3(source.getHeadLocation(), multiplyVector3Number(source.getViewDirection(), MAX_RANGE));
            ...
            for (let i = 1; i <= dist; i++) {
                system.runTimeout(() => {
                    const pos = addVector3(sourcePos, multiplyVector3Number(sourceDir, i));
                    ...
                    system.runTimeout(() => {
                        ...
                        const entities = [
                            ...new Map([...entitiesByRange, ...entitiesAtBlock]
                        ...
                        for (let j = 0; j < entities.length; j++) {
                            ...
                            if (isFamily(entity, "monster")) {
                                entity.applyDamage(isFamily(entity, "undead") ? UNDEAD_DAMAGE : MONSTER_DAMAGE, {
                                    damagingEntity: data.source,
                                    cause: EntityDamageCause.magic,
                                });
                                dimension.spawnParticle("minecraft:critical_hit_emitter", entity.getHeadLocation());
                                dimension.spawnParticle("minere:emerald_wave", entity.location);
                            }
                            else {
                                entity.addEffect("instant_health", 1);
                                dimension.spawnParticle("minecraft:heart_particle", entity.getHeadLocation());
                            }
                        }
...
function growBlocks(dimension, pos) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        if (growables &&
            ...
            dimension.spawnParticle("minecraft:crop_growth_emitter", block.above().location);
        }
    }
}
