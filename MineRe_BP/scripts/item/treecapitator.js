import { EntityComponentTypes, EquipmentSlot, system, BlockVolume, ItemComponentTypes, } from "@minecraft/server";
...
function treecapitate(dimension, origin, durability) {
    ...
    while (!queue.isEmpty()) {
        ...
        const neighbors = getNeighbors(dimension, next, {
        ...
        for (let i = 0; i < neighbors.length; i++) {
            ...
            if (checkRange(origin, neighbor, LOG_BFS_HORIZONTAL_LIMIT + // Use the stricter limit here
                (altitude > ALTITUDE_BONUS_START ? ALTITUDE_BONUS : 0), MAX_VERTICAL_DISTANCE) &&
            ...
            checkRange(origin, neighbor, LEAF_HORIZONTAL_DISTANCE +
                (altitude > ALTITUDE_BONUS_START ? ALTITUDE_BONUS : 0), MAX_VERTICAL_DISTANCE) &&
        ...
        system.runTimeout(() => {
            for (let j = 0; j < logLevelToBreak[i]?.length; j++) {
                ...
                if (isValid(dimension, logPos)) {
                    dimension.runCommand(`setBlock ${logPos.x} ${logPos.y} ${logPos.z} air destroy`);
    ...
    system.runTimeout(() => {
        ...
        while (!leafQueue.isEmpty()) {
            ...
            if (checkIsType(dimension, nextLeaf, leavesAndMushrooms, [
                "leaves",
                "shroom_block",
                "wart_block",
                "shroomlight",
                ...
                dimension.runCommand(`setBlock ${nextLeaf.x} ${nextLeaf.y} ${nextLeaf.z} air destroy`);
            }
            // Check neighbors of the current leaf to find more connected leaves
            const leafNeighbors = getNeighbors(dimension, nextLeaf, { x: -1, y: -1, z: -1 }, { x: 1, y: 1, z: 1 });
...
export function runTreecapitate(player, location, blockPermutation, treeCapitator) {
    if (!player) {
        return 0;
    }
    ...
    const neighbors = getNeighbors(dimension, location, {
        x: -1,
        y: 0,
        z: -1,
    }, {
    ...
    const logsBroken = treecapitate(player.dimension, location, durability?.maxDurability - durability?.damage);
    return logsBroken;
}
export function offHandTreecapitate(data) {
    ...
    const logsBroken = runTreecapitate(data.player, data.block.location, data.brokenBlockPermutation, treeCapitator);
...
export const Treecapitator = {
    onMineBlock(event) {
        system.runTimeout(() => {
            const logsBroken = runTreecapitate(event.source, event.block.location, event.minedBlockPermutation, event.itemStack);
            if (logsBroken > 0) {
                reduceDurability(event.source, event.itemStack, logsBroken + 1, // +1 for the initial block broken
                EquipmentSlot.Mainhand);
            }
            else {
