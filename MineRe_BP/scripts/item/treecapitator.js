import { world, EntityComponentTypes, EquipmentSlot, system, BlockVolume, ItemComponentTypes, } from "@minecraft/server";
...
function checkWidthWithOffset(dimension, origin, yOffset) {
    ...
    for (let i = 0; i <= HORIZONTAL_CHECK_DISTANCE; i++) {
        const log = checkIsType(dimension, {
            x: origin.x + i,
            y: origin.y + yOffset,
            z: origin.z,
        }, logSet, ["log", "stem"]);
...
function checkWidth(dimension, origin) {
    let max = 0;
    for (let i = -1 * VERTICAL_CHECK_DISTANCE; i <= VERTICAL_CHECK_DISTANCE; i++) {
...
function treecapitate(dimension, origin, durability) {
    ...
    while (!queue.isEmpty()) {
        ...
        for (let i = 0; i < neighbors.length; i++) {
            ...
            visited.add(neighborString);
            if (checkRange(origin, neighbor, LEAF_HORIZONTAL_DISTANCE, MAX_VERTICAL_DISTANCE) &&
                checkIsType(dimension, neighbor, leavesAndMushrooms, [
                    "leaves",
                    "shroom_block",
                    "wart_block",
    ...
    system.runTimeout(() => {
        leaves.forEach((leaf) => {
            dimension.runCommand(`setBlock ${leaf.x} ${leaf.y} ${leaf.z} air destroy`);
        });
...
export const Treecapitator = {
    onMineBlock(event) {
        system.runTimeout(() => {
            ...
            if (logsBroken > 0) {
                reduceDurability(event.source, event.itemStack, logsBroken + 1, EquipmentSlot.Mainhand);
            }
            else {
