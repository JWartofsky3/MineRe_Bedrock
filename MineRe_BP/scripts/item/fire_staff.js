    ...
    if (itemStack.typeId == "minere:fire_staff") {
        system.run(() => {
            if (source.isSneaking) {
                ...
                if (!source.runCommand("clear @s[m=!c] fire_charge 0 4").successCount &&
                ...
                if (!source.runCommand("clear @s[m=!c] fire_charge 0 1").successCount &&
                    source.getGameMode() !== "creative") {
                    source.playSound("item.amethyst_staff.error");
                    return;
                }
                ...
                let fireball = source.dimension.spawnEntity("minere:staff_fireball", loc);
                ...
                fireball.applyImpulse(addVector3(multiplyVector3Number(source.getViewDirection(), 2.25), randomVector3(0.05)));
...
function fireWall(position, dimension, rings, spacing, height) {
    const minY = Math.max(dimension.heightRange.min + 1, position.y + 1 - height / 2);
    const maxY = Math.min(dimension.heightRange.max - 1, position.y + 1 + height / 2);
    let timeout = 3;
    ...
    for (let i = 1; i <= rings; i++) {
        system.runTimeout(() => {
            ...
            // z axis
            dimension.runCommand(`fill ${spacing * i + position.x} ${minY} ${spacing * i + position.z} ${-spacing * i + position.x} ${maxY} ${spacing * i + position.z} fire replace air`);
            dimension.runCommand(`fill ${spacing * i + position.x} ${minY} ${-spacing * i + position.z} ${-spacing * i + position.x} ${maxY} ${-spacing * i + position.z} fire replace air`);
            // x axis
            dimension.runCommand(`fill ${spacing * i + position.x} ${minY} ${spacing * i + position.z} ${spacing * i + position.x} ${maxY} ${-spacing * i + position.z} fire replace air`);
            dimension.runCommand(`fill ${-spacing * i + position.x} ${minY} ${spacing * i + position.z} ${-spacing * i + position.x} ${maxY} ${-spacing * i + position.z} fire replace air`);
