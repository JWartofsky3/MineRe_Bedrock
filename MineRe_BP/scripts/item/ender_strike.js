...
export const applyEnderStrike = (data) => {
    ...
    //apply multi hits
    const entities = getNearbyEntities(data.attackingEntity, data.hitEntity, stats);
...
function getNearbyEntities(attacker, targetEntity, stats) {
    const typeFamily = targetEntity?.getComponent(EntityComponentTypes.TypeFamily);
...
function enderEffects(entity) {
    ...
    for (let k = 0; k < 30; k++) {
        entity?.dimension.spawnParticle("minecraft:end_chest", addVector3(entity.location, randomVector3(1)));
    }
}
