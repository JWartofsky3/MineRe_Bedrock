    ...
    if (itemStack.typeId == "minere:amethyst_staff") {
        system.run(() => {
            if (source.isSneaking) {
                ...
                if (!source.runCommand(`clear @s[m=!c] amethyst_shard 0 ${getRandomIntInclusive(2, 3)}`).successCount &&
                ...
                if (source.getGameMode() !== GameMode.Creative &&
                    findItemInContainer(source.getComponent(EntityComponentTypes.Inventory)?.container, "minecraft:amethyst_shard") === -1) {
                    source.playSound("item.amethyst_staff.error");
                    return;
                }
