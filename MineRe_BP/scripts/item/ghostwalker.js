...
export const Ghostwalker = {
    onHitEntity(arg) {
        if (!arg.attackingEntity?.isValid ||
        ...
        const lore = pwns < MAX_LEVEL
            ? `§c☠♠⚔ §c(${pwns}) §c⚔♠☠`
            : `§c☠♠⚔ §6(${pwns}) §c⚔♠☠`;
        arg.itemStack.setLore([lore]);
        const equippable = arg.attackingEntity.getComponent(EntityComponentTypes.Equippable);
...
function getShadowTime(level) {
    ...
    const shadowTime = SHADOW_TIME_MIN + (SHADOW_TIME_MAX - SHADOW_TIME_MIN) * (level / MAX_LEVEL);
...
function getJumpBoost(level) {
    ...
    const jumpBoost = JUMP_BOOST_MIN + (JUMP_BOOST_MAX - JUMP_BOOST_MIN) * (level / MAX_LEVEL);
