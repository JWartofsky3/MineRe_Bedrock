import { system, EntityDamageCause, Player, } from "@minecraft/server";
import { GetBossTarget, SetBossTarget } from "./boss_utils";
import { addVector3, directionVector3, distVector3, getRandomAir, magnitudeVector3, multiplyVector3Number, } from "util/vector3Functions";
...
(function (InfernoMode) {
    InfernoMode[InfernoMode["Ranged"] = 0] = "Ranged";
    InfernoMode[InfernoMode["Melee"] = 1] = "Melee";
    InfernoMode[InfernoMode["Push"] = 2] = "Push";
    InfernoMode[InfernoMode["Stomp"] = 3] = "Stomp";
    InfernoMode[InfernoMode["Guard"] = 4] = "Guard";
    InfernoMode[InfernoMode["Stunned"] = 5] = "Stunned";
})(InfernoMode || (InfernoMode = {}));
// ====== CORE LOGIC ======
function setMode(boss, mode, allowOverrideStunned = false) {
...
function pickWeightedMode(modes, boss, allowStunned = false, allowGuard = false) {
    const currentMode = boss.getProperty(BOSS_MODE_PROPERTY);
    if (currentMode === InfernoMode.Stunned && !allowStunned)
        return;
    if (currentMode === InfernoMode.Guard && !allowGuard)
        return;
    const availableModes = modes.filter((m) => m.mode !== currentMode && m.weight > 0);
...
// ====== MODE BEHAVIORS ======
function rangedMode(boss, allowOverrideStunned = false) {
    setMode(boss, InfernoMode.Ranged, allowOverrideStunned);
}
function meleeMode(boss) {
...
function startPush(boss, allowOverrideStunned = false) {
    if (boss.getProperty(BOSS_MODE_PROPERTY) === InfernoMode.Push)
        return;
...
function guardMode(boss, allowOverrideStunned = false) {
    if (boss.getProperty(BOSS_MODE_PROPERTY) === InfernoMode.Guard)
        return;
    const target = GetBossTarget(boss);
    if (!target)
        return rangedMode(boss);
...
function becomeStunned(boss) {
    if (boss.getProperty(BOSS_MODE_PROPERTY) === InfernoMode.Stunned)
        return;
...
function exitStun(boss) {
    const target = GetBossTarget(boss);
    boss.setDynamicProperty(STUN_DAMAGE_PROPERTY, 0);
    if (!target)
        return rangedMode(boss, true);
...
function startStrafeRunner(entity) {
...
function startRunner(entity) {
    ...
    const runner = system.runInterval(() => {
        ...
        const currentMode = entity.getProperty(BOSS_MODE_PROPERTY);
        if (currentMode !== InfernoMode.Push &&
...
// ====== EVENT LISTENERS ======
export function startInfernoRunners(entity) {
    if (isAliveInferno(entity))
        startRunner(entity);
}
export function infernoOnHurtEntity(data) {
    onInfernoHurt(data);
    onHurtByInferno(data);
}
function onInfernoHurt(data) {
...
function onHurtByInferno(data) {
    ...
    const target = GetBossTarget(boss);
    if (data.damageSource?.damagingProjectile?.typeId === "minere:blue_fireball") {
        if (!!target?.getEffect("fire_resistance")) {
            ...
            if (target instanceof Player) {
                ...
                spawnParticleCloud("minere:skull_particles", target.location, 2, 20, target.dimension);
            }
        }
    }
...
// ====== UTILS ======
function isAliveInferno(entity) {
    return (!!entity &&
        entity.isValid &&
        entity.typeId === INFERNO_TYPE_ID &&
        isAlive(entity));
}
function clearRun(boss, runPropertyName) {
    try {
...
export function applyStrafe(entity, target, force) {
...
function teleportToTarget(boss) {
    if (!isAlive(boss))
        return;
    const currentMode = boss.getProperty(BOSS_MODE_PROPERTY);
    ...
    const projected = addVector3(boss.location, multiplyVector3Number(dir, moveDistance));
...
function teleportParticleLine(boss, location, faceLocation) {
    if (!isAlive(boss)) {
        return;
    }
    clearRun(boss, RETREAT_RUNNER);
    ...
    for (let i = 0; i <= distance; i++) {
        spawnParticleCloud("minecraft:candle_flame_particle", addVector3(boss.location, multiplyVector3Number(dir, i)), 2, 10, dimension);
    }
