...
// ====== CORE LOGIC ======
function setMode(boss, mode, allowStunned = false) {
    if (boss.getProperty(BOSS_MODE_PROPERTY) === mode)
        return;
    if (!allowStunned &&
...
function pickWeightedMode(modes, boss) {
    const currentMode = boss.getProperty(BOSS_MODE_PROPERTY);
    if (currentMode === InfernoMode.Stunned)
        return;
...
function startPush(boss, allowStunned = false) {
...
function guardMode(boss, allowStunned = false) {
...
function teleportToTarget(boss) {
    ...
    if (distanceToTarget > 16) {
        targetZone = addVector3(boss.location, multiplyVector3Number(toTargetDir, 16));
    }
...
function teleportParticleLine(boss, location, faceLocation) {
    ...
    for (let i = 0; i <= distance; i++) {
        spawnParticleCloud("minecraft:candle_flame_particle", addVector3(boss.location, multiplyVector3Number(dir, i)), 2, 10, boss.dimension);
    }
