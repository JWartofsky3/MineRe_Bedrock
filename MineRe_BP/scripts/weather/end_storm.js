...
function runStorm(player) {
    ...
    const lightningInterval = getRandomIntInclusive(LIGHTNING_PROPERTIES.INTERVAL_MIN, LIGHTNING_PROPERTIES.INTERVAL_MAX);
    const stormRunner = system.runInterval(() => {
        const currentPlayer = getActivePlayerById(id);
        if (!currentPlayer || !isInEndStormZone(currentPlayer))
            return;
    ...
    const stormDuration = getRandomIntInclusive(STORM_PROPERTIES.MIN_DURATION, STORM_PROPERTIES.MAX_DURATION);
    system.runTimeout(() => {
        system.clearRun(stormRunner);
        activeStorms.delete(id);
    }, secondsToTicks(stormDuration));
}
function getActivePlayerById(id) {
    return [...world.getPlayers()].find((p) => p.id === id);
}
function triggerLightningStrikes(player, dimension, canBeStruck) {
    const strikesCount = getRandomIntInclusive(LIGHTNING_PROPERTIES.MIN_STRIKES, LIGHTNING_PROPERTIES.MAX_STRIKES);
    ...
    for (let i = 0; i < strikesCount; i++) {
        const strikeDelay = getRandomIntInclusive(LIGHTNING_PROPERTIES.MIN_STRIKE_DELAY, LIGHTNING_PROPERTIES.MAX_STRIKE_DELAY);
        system.runTimeout(() => {
            const currentPlayer = getActivePlayerById(player.id);
            if (!currentPlayer || !isInEndStormZone(currentPlayer))
                return;
            const lightningPos = calculateLightningPosition(currentPlayer, dimension, canBeStruck);
            if (lightningPos) {
...
function calculateLightningPosition(player, dimension, canBeStruck) {
    ...
    const groundY = dimension.getTopmostBlock(player.location)?.location.y ?? minY;
    const aboveGround = playerY - groundY;
    // Direct hit case
    if (aboveGround > LIGHTNING_PROPERTIES.HIT_HEIGHT &&
    ...
    // Strike near player
    const randomPos = addVector3(player.getHeadLocation(), randomVector3(LIGHTNING_PROPERTIES.MAX_DISTANCE));
...
function runEndFlak(player, dimension, canBeStruck) {
    let canBeHit = () => {
        return (canBeStruck &&
    ...
    const strikes = getRandomIntInclusive(END_FLAK_PROPERTIES.MIN, END_FLAK_PROPERTIES.MAX);
    for (let i = 0; i < strikes; i++) {
        const delay = getRandomIntInclusive(LIGHTNING_PROPERTIES.MIN_STRIKE_DELAY, LIGHTNING_PROPERTIES.MAX_STRIKE_DELAY);
        system.runTimeout(() => {
            if (!canBeHit()) {
                return;
            }
            const targetPos = addVector3(player.location, randomVector3(END_FLAK_PROPERTIES.MAX_DISTANCE));
            spawnParticleCloud("minecraft:end_chest", targetPos, END_FLAK_PROPERTIES.PARTICLE_DISTANCE, END_FLAK_PROPERTIES.PARTICLE_COUNT, dimension);
            spawnParticleCloud("minecraft:explosion_manual", targetPos, END_FLAK_PROPERTIES.PARTICLE_DISTANCE, END_FLAK_PROPERTIES.PARTICLE_COUNT, dimension);
            ...
            if (dist < END_FLAK_PROPERTIES.DAMAGE_DIST) {
                const flakDamage = Math.min(END_FLAK_PROPERTIES.DAMAGE_MULT *
                    (END_FLAK_PROPERTIES.DAMAGE_DIST +
                        1 -
                        END_FLAK_PROPERTIES.DAMAGE_DIST), END_FLAK_PROPERTIES.DAMAGE_CAP);
...
function isInEndStormZone(player) {
    if (!player) {
        return false;
    }
    ...
    if (distance < STORM_PROPERTIES.MIN_DISTANCE)
        return false;
    ...
    if (distance >= bandStart && distance < bandEnd)
        return false;
