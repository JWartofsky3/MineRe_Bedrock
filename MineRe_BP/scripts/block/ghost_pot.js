...
export const ghostPot = {
    onPlayerBreak(arg) {
        ...
        dimension.runCommand(`loot spawn ${location.x} ${location.y} ${location.z} loot "blocks/ghost_pot"`);
        // X +
        for (let i = 0; i < GHOST_COUNT; i++) {
            if (Math.random() <= GHOST_CHANCE) {
                system.runTimeout(() => {
                    try {
                        spawnGhost(dimension, addVector3(addVector3(location, randomVector3(GHOST_SPREAD)), {
                            x: GHOST_DISTANCE,
                            y: 0,
                            z: 0,
                        }));
...
function spawnGhost(dimension, location) {
