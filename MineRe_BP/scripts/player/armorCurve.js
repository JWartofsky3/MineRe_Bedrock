...
export const armorCurve = (player, damage, damageSource) => {
    ...
    health.setCurrentValue(Math.min(health.currentValue + armorCurveDiff + protectionDiff, health.effectiveMax));
};
...
function getProtectionDiff(equippable, damage, cause) {
    if (hasEnchantment(equippable, "fire_protection")) {
        if (cause == EntityDamageCause.fire ||
    ...
    if (hasEnchantment(equippable, "blast_protection")) {
        if (cause == EntityDamageCause.entityExplosion ||
            cause == EntityDamageCause.blockExplosion ||
            cause == EntityDamageCause.fireworks) {
            return 0;
        }
    ...
    const totalProt = getProtectionValue(equippable, EquipmentSlot.Head) +
...
function getProtectionValue(equippable, slot) {
    ...
    const itemEnchantableComponent = item.getComponent(ItemComponentTypes.Enchantable);
    if (!itemEnchantableComponent) {
        return 0;
    }
...
function getDefenseDifference(equippable, slot) {
    ...
    switch (slot) {
        ...
        case EquipmentSlot.Chest:
            return (defenseMap.get("minecraft:diamond_chestplate") -
                defenseMap.get(item.typeId) +
                materialBonus);
        case EquipmentSlot.Legs:
            return (defenseMap.get("minecraft:diamond_leggings") -
                defenseMap.get(item.typeId) +
                materialBonus);
        case EquipmentSlot.Feet:
            return (defenseMap.get("minecraft:diamond_boots") -
                defenseMap.get(item.typeId) +
                materialBonus);
    }
    return 0;
