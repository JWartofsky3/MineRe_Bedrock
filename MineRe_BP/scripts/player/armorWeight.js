...
export function armorWeight(player) {
    ...
    // JUMP BOOTS
    if (equippable.getEquipmentSlot(EquipmentSlot.Feet).typeId === "minere:jump_boots") {
    ...
    const finalSpeedValue = BASE_MOVEMENT *
        baseMoveSpeed *
...
function getSneakMultiplier(player) {
    ...
    if (enchantable.hasEnchantment("swift_sneak")) {
        return Math.min(1.0, SNEAK_MULT + 0.15 * enchantable.getEnchantment("swift_sneak").level);
    }
    return SNEAK_MULT;
}
