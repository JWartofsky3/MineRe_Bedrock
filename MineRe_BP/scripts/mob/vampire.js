...
import { addVector3, multiplyVector3Number, getRandomAir, } from "util/vector3Functions";
...
export function vampireHeal(vampire, target) {
    ...
    vampireHealth.setCurrentValue(Math.min(vampireHealth.effectiveMax, vampireHealth.currentValue + VAMPIRE_HEAL_AMOUNT));
}
export function vampireHurt(vampire, attacker) {
    ...
    const attackerEquippable = attacker.getComponent(EntityComponentTypes.Equippable);
    ...
    if (weapon.typeId.toLowerCase().includes("gold")) {
        vampireHealth.setCurrentValue(vampireHealth.currentValue - GOLD_DAMAGE_BOUNUS);
    }
}
export function rollBecomeBat(entity, chance, minHealth) {
    ...
    if (health.currentValue > minHealth * health.effectiveMax ||
