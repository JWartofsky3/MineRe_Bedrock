...
import { addVector3, multiplyVector3Number, directionVector3, distVector3, randomVector3, } from "util/vector3Functions";
import { DEFAULT_TICK } from "main";
export function rollCastFire(caster, target, chance) {
    if (!caster || !target) {
        return;
    }
    const FIRE_COOLDOWN = "fireCooldown";
    const activationRange = 6; // min range to activate
        ...
        for (let i = 1; i <= maxRange; i++) {
            system.runTimeout(() => {
                ...
                for (let k = 0; k < 3; k++) {
                    ...
                    const pos = addVector3(addVector3(addVector3(caster.location, multiplyVector3Number(dir, i)), randomVector3(1.5)), { x: 0, y: 1.0, z: 0 });
                    ...
                    for (let j = -4; j < 8; j++) {
                        ...
                        if (block.isValid &&
                            ...
                            target.dimension.runCommand(`setblock ${block.location.x} ${block.location.y} ${block.location.z} fire`);
                            isSuccessful = true;
                            break;
                        }
                    }
